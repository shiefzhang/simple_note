...
### Web 1.1.5（2026-08-27）

- 修复搜索激活时切换 Markdown/源码模式后当前命中高亮消失的问题；
- 搜索激活时只使用当前命中项恢复位置，避免通用滚动锚点异步覆盖搜索定位；
- 切回源码模式时聚焦编辑器，并让源码与预览中的当前命中保持居中。

### Web 1.1.4（2026-08-27）

- 修复页内搜索后源码编辑器被只读搜索预览替换、无法继续编辑的问题；
- 源码模式直接选中当前匹配，编辑时刷新匹配结果但不抢走光标；
- Markdown 和 HTML 预览保持只读并显示搜索高亮，清空搜索时同步移除高亮；
- 在源码、Markdown 和 HTML 模式之间切换时保留当前匹配和阅读位置；
- Android 内置编辑器同步相同的搜索行为与位置锚点修复；
- 将静态资源缓存版本统一更新为 `1.1.4`。

### Web 1.1.3（2026-07-05）

- 左侧栏版本号改为由运行中的 FastAPI 后端通过 `/api/version` 返回；
- 版本接口禁止缓存，前端请求时附加时间戳，避免代理或浏览器返回旧版本；
- 若只更新静态文件但没有更新并重启后端，左侧栏显示“后端版本未知”；
- 看到 `Web v1.1.3` 才表示当前 Python 后端代码已经更新并重启。

### Web 1.1.2（2026-07-05）

- 修复网页版 Markdown 表格因旧静态资源缓存而看起来未渲染的问题；
- 为 Marked、应用脚本和样式表统一增加 `1.1.2` 缓存版本；
- Markdown 表格统一包裹横向滚动容器，避免宽表格挤压编辑区；
- 左侧栏底部增加版本展示区域；从 1.1.3 起版本值改由后端动态提供。
...
|---|---|---|
| GET | `/` | 返回网页 |
| GET | `/api/version` | 返回当前运行中的后端版本；响应禁止缓存 |
